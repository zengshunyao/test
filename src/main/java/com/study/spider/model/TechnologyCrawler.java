...
import com.study.spider.pojo.Technology;
...
@Deprecated
public class TechnologyCrawler extends WebCrawler {
    ...
    public static List<Technology> technologyList = new LinkedList<Technology>();
    ...
    public boolean shouldVisit(Page referringPage, WebURL url) {
        ...
        return !IGNORE_URL_FILTERS.matcher(href).matches()
                && href.startsWith("http://www.sckjfp.com/Index/techSupplySearch/p/")
                && href.matches("^http://www.sckjfp.com/Index/techSupplySearch/p/(\\d){1,}.html$");
    }
            ...
            for (int i = 0; i < trList.size(); i++) {
                ...
                for (int j = 0; j < tdList.size(); j++) {
                    ...
                    if (j >= 5) {
                        ...
                        try {
                            Field field = Technology.ORDER_MAP.get(j + 2);
                            Method setMethod = Technology.class.getMethod("set"
                                    + field.getName().substring(0, 1).toUpperCase()
                                    + field.getName().substring(1), String.class);
                            setMethod.invoke(technology, td.text());
