 ...
 * 风骚的Michael 老师
 */
public class CreateSessionDemoFromVip implements Watcher {
    ...
    public static void main(String[] args) {
        try {
            zookeeper = new ZooKeeper(CONNECTSTRING, 5000, new CreateSessionDemoFromVip());

            countDownLatch.await();
            //zookeeper.delete("/node2",-1);
            zookeeper.create("/node2", "111".getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL);

            zookeeper.getData("/node2", true, new Stat());
...
//        Thread.sleep(2000);
            //修改数据
            zookeeper.setData("/node2", "mic234".getBytes(), -1);
            zookeeper.setData("/node2", "mic234".getBytes(), -1);
            Thread.sleep(1000);
...
//            TimeUnit.SECONDS.sleep(1);
            ...
            //获取指定节点下的子节点
            List<String> childrens = zookeeper.getChildren("/bug", true);
            ...
            if (Watcher.Event.EventType.None == watchedEvent.getType() && null == watchedEvent.getPath()) {
                ...
                try {
                    System.out.println("数据变更触发路径：" + watchedEvent.getPath() + "->改变后的值：" +
                            zookeeper.getData(watchedEvent.getPath(), false, stat));
                } catch (KeeperException e) {
                    e.printStackTrace();
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
            } else if (watchedEvent.getType() == Watcher.Event.EventType.NodeChildrenChanged) {//子节点的数据变化会触发
                try {
