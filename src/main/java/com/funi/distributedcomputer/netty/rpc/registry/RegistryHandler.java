...
public class RegistryHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        //客户端传过来的调用信息
        InvokerMsg request = null;
        if (msg instanceof InvokerMsg) {
            request = (InvokerMsg) msg;
            if (registryMap.containsKey(request.getClassName())) {
                Object object = registryMap.get(request.getClassName());

                Method m = object.getClass().getMethod(request.getMethodName(), request.getParames());
                result = m.invoke(object, request.getValues());
            }
        }
